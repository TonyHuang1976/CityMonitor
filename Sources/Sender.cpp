...
*/
        ...
        int availLen = buffer->GetValidDataLen();
        if (availLen == 0)
        {
#ifdef __WIN32__
            Sleep(100);
        ...
        if (availLen > 0)
        {
            ...
            //uint len = send( socket, (char*)sendBuffer, availLen, MSG_OOB );  // MSG_OOB or MSG_PEEK ?
            int len = send( socket, (char*)sendBuffer, availLen, 0 );   // MSG_OOB or MSG_PEEK ?
            if (len <= 0)
            {
#ifdef __WIN32__
...
/*
    ...
    uint availLen = buffer->GetValidDataLen();
        if (availLen > 0)
        {
            ...
            buffer->Read(sendBuffer, availLen);
            // 从 sendBuffer 中发送 availLen 个字节数据到左邻节点 
            uint len = SendAll(socket, (char*)sendBuffer, availLen );   // MSG_OOB or MSG_PEEK ?
            if (len < availLen){
          break; //出错
            }
      } 
